...
void example_ring_buffer()
{
	...
	//※一部リングバッファ固有のテストを行うが、基本的に動的配列のテストとほとんど同じ
	{
	...
	#ifdef TEST_ITERATOR_OPERATION
		{
			...
			if (ite.isExist()) std::printf("ite:[%d(%d)] key=%d, value=%d\n", static_cast<int>(ite.getIndex()), ite.getRealIndex(), ite->m_key, ite->m_val);
			if (rite.isExist()) std::printf("rite:[%d(%d)] key=%d, value=%d\n", static_cast<int>(rite.getIndex()), rite.getRealIndex(), rite->m_key, rite->m_val);
			if (ite_end.isExist()) std::printf("ite_end:[%d(%d)] key=%d, value=%d\n", static_cast<int>(ite_end.getIndex()), ite_end.getRealIndex(), ite_end->m_key, ite_end->m_val);
			if (rite_end.isExist()) std::printf("rite_end:[%d(%d)] key=%d, value=%d\n", static_cast<int>(rite_end.getIndex()), rite_end.getRealIndex(), rite_end->m_key, rite_end->m_val);
			if (ite2.isExist()) std::printf("ite2:[%d(%d)] key=%d, value=%d\n", static_cast<int>(ite2.getIndex()), ite2.getRealIndex(), ite2->m_key, ite2->m_val);
			if (rite2.isExist()) std::printf("rite2:[%d(%d)] key=%d, value=%d\n", static_cast<int>(rite2.getIndex()), rite2.getRealIndex(), rite2->m_key, rite2->m_val);
			if (ite2_end.isExist()) std::printf("ite2_end:[%d(%d)] key=%d, value=%d\n", static_cast<int>(ite2_end.getIndex()), ite2_end.getRealIndex(), ite2_end->m_key, ite2_end->m_val);
			if (rite2_end.isExist()) std::printf("rite2_end:[%d(%d)] key=%d, value=%d\n", static_cast<int>(rite2_end.getIndex()), rite2_end.getRealIndex(), rite2_end->m_key, rite2_end->m_val);
