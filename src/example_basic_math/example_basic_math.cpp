...
void printCalcBits(const unsigned int (&values)[N])
{
	for (unsigned int i = 0; i < N; ++i)
	{
		...
		std::printf("0x%08x : count=%2d, MSB=%2d, LSB=%2d, near-pow2(GE)=0x%08x, near-pow2(LE)=0x%08x, adjustAlign<ALIGN>=0x%08llx,0x%08llx(ALIGN=4,256)\n", value, countBits(value), calcMSB(value), calcLSB(value), calcNearPow2GE(value), calcNearPow2LE(value), static_cast<unsigned long long>(adjustAlign<4>(value)), static_cast<unsigned long long>(adjustAlign<256>(value)));
	}
}
...
{
	std::printf("0x%08x : count=%2d, MSB=%2d, LSB=%2d, near-pow2(GE)=0x%08x, near-pow2(LE)=0x%08x, adjustAlign<ALIGN>=0x%08llx,0x%08llx(ALIGN=4,256)\n", N, countStaticBits<N>::value, calcStaticMSB<N>::value, calcStaticLSB<N>::value, calcStaticNearPow2GE<N>::value, calcStaticNearPow2LE<N>::value, static_cast<unsigned long long>(adjustStaticAlign<N, 4>::value), static_cast<unsigned long long>(adjustStaticAlign<N, 256>::value));
}
