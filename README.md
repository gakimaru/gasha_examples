...
> C++の「ゼロオーバーヘッドの原則」に倣い、不要な処理がパフォーマンスに影響しないように設計しています。  
> 例えば、コンテナクラスに対するマルチスレッド用のロック制御を、テンプレート引数によるポリシーパターンで実装しています。これにより、ロック制御不要な時は、何もしないダミーのロッククラスを与える事で、ロック制御の処理を無効化する仕組みです。コンパイラの最適化により、実際に処理コードが完全に消滅します。  
> 
> また、メタプログラミングの対応も強化し、コンパイル時のCRC計算や素数計算などに対応し、実行時のパフォーマンス向上を図ることができます。  
