...
**開発環境も考慮しています。**  
> Gitのサブモジュールを活用し、多数のプロジェクトでライブラリを共通利用する環境を想定してリポジトリを構成しています。  
> プロジェクトごとのライブラリの動作要件をカスタマイズし易いように構成しています。  

**名前の由来**  
> 名前の由来は「[がしゃどくろ][GASHADOKURO]」から。なので、読み方は「ガシャ」。  
> その「骨格」的な意味合い（？）がライブラリを象徴し、自身のハンドル名「Gakimaru」の頭文字「GA」を重ねて命名しました。  
> 頭文字の「GA」は、「GAme向け」という解釈でもかまいません。  
...
--------
> 注釈
> * *T 以降の説明で、「*T」の注釈記号が付いたライブラリは、特殊なクラスライブラリです。  
> コンパイル・リンク時間の短縮、および、クラス修正時の影響範囲の抑制（再コンパイル対象の抑制）のために、ソースファイルを「宣言部」（.h）、「インライン関数／テンプレート関数定義部」（.inl）、「関数定義部」（.cpp.h）の三つに分けています。  
> それぞれ適切な場所でインクルードし、テンプレートクラスの明示的なインスタンス化を行う事で、ビルドを最適化できます。  
> なお、面倒なら全部まとめてインクルードすれば、明示的なインスタンス化をせずに使用可能です。  

####環境####
* `gasha/build_settings/build_settings.h` **ビルド設定統括** *1  
* `gasha/build_settings/common/compiler_auto_settings.h` **コンパイラ自動判別＆設定**  
* `gasha/build_settings/common/platform_auto_settings.h` **プラットフォーム自動判別＆設定**  
* `gasha/build_settings/common/language_auto_settings.h` **言語機能自動判別＆設定**  
...
> * プロジェクト固有のカスタマイズが、対象プラットフォーム上で動作するかチェックする関数も用意しています。（現状GCCは非対応）  
> * 現状、x86,x64系CPU／Windows+VC++2013／Cygwin+GCC4.8.2の環境でしか動作確認しておらず、他の環境に適合する状態ではありませんが、対象環境を拡張していけるように構成しています。  
> 
> > 注釈  
> > * *1：build_settings.h が他の設定ファイルを全てインクルードしています。
> > 通常、このファイルを強制インクルードファイルに含めて利用します。プリコンパイル済みヘッダーに含めることも推奨します。  
> > * *2：ライブラリに対するプロジェクト固有のカスタマイズを行います。  
> > 例えば「SSE命令をどこまで使用するか？」、「CRC-32の生成多項式はCRC-32Cを採用するか？」、「コンテナクラスの明示的インスタンス化を行わなくても使用可能とするか？」といった設定を行います。  
> > * *2+：対象マシン上で、カスタマイズ要件通りの動作が可能かチェックする関数も用意しています。  
> > 例えば、SSE4.2命令の使用が設定されていれば、そのマシン上でSSE4.2命令が使用可能か判定します。  
> > * *2++：この設定ファイルはライブラリ本体とは別のリポジトリで管理しており、ユーザー側に更新権限があるものとして位置付けます。  
> > *3：ライブラリのネームスペースを独自にカスタマイズできます。  
> > *4：ライブラリの挙動を設定するためのマクロを有効化／無効化してカスタマイズします。  
> 
> 資料  
