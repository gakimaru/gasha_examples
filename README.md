...
> ご自由にお使い下さい。  
> C++11の新しい構文、ライブラリもふんだんに取り入れていますので、参考程度にでもご活用頂ければ幸いです。  
...
> とくにマルチスレッド処理の共通性が高く、CPU命令に対する依存が大きいロックフリーアルゴリズムも、汎用的な処理コードで実現しています。  

**ゲーム開発での利用を想定し、基本的なアルゴリズム、コンテナ、メモリ管理、デバッグ機能などを用意しています。**  
> 「ゲーム開発向け」と謳いながらも、扱っているのはゲーム開発特有のグラフィック処理や3D演算などではなく、もっと基本的なプログラミング要素が主体です。  
> STLライクなコンテナは、データ構造とアルゴリズムを分離した構成としており、ユーザー独自のデータを一時的にコンテナ化して操作するなど、柔軟に活用することができます。  
> 「STLは暗黙的なメモリ確保があるのでゲーム開発では扱いにくい」といった問題への対処や、「コンテナは使わず平衡木のアルゴリズムだけ流用したい」といった要件への対応として、扱うことができるライブラリです。  
