...
> 例えば、**「STLは暗黙的なメモリ確保があるのでゲーム開発では扱いにくい」**といった問題の対処として、STLライクな固定バッファのコンテナや、標準new/deleteを任意のアロケータに置き換える手段を用意しています。  
...
> 例えば、「優先度付きキュー」などの幾つかのクラスはマルチスレッド用の排他制御に対応していますが、それが不要な場合（シングルスレッド利用時など）、排他制御の処理コードがコンパイル時に完全に消滅する仕組みです。  
>（注：インライン展開などのコンパイラの最適化が十分に機能することによって、完全な消滅となります）。  
> また、高速化のために、テンプレートを活用したメタプログラミングも強化しています。  

**マルチプラットフォームでソースコードを共通利用するためのライブラリ構成です。**  
> コンパイル時にコンパイラの種類とバージョンを自動判別し、コンパイラ固有仕様をC++11仕様に偽装するなどして、ソースコードレベルでの共通性を高めます。  
> また、対象プラットフォームも自動判別し、コンパイルスイッチ（実効コードの切り替え）に対応します。  
...
> 標準的なコンパイラのマクロ（_DEBUGなど）に基づいて、ライブラリ内で統一的なビルド構成用マクロを定義します。  
...
> ただし、幾つか標準ライブラリと同名の関数やクラスがあります。そのため、（GASHAに限らず内製ライブラリなどを意図通りに使用するためには）`using`ディレクティブの指定は、特によく使用するライブラリに限定し、他はネームスペースを明示して使用した方が無難です。  
> GASHAのソースコードでは、標準ライブラリ使用時は `std::` を明示するルールとしています。  
> また、コンパイル時のインクルード探索パスには `gasha/` を含めないようにすれば、`#include` に `gasha/` の指定が必須となり、インクルードファイルの競合が起こりません。  
...
> *注：各サンプルプログラムは、開発過程で機能やパフォーマンスの評価に使用したものです。  
> 網羅的な機能チェックで長い処理になっているものもあり、難解かもしれません。  
...
* `gasha/build_settings/build_settings_diag.h` **プロジェクト固有カスタマイズ設定（最終設定）** ※3 ※5  
...
> * **※3**(1)：対象マシン上で、カスタマイズ要件通りの動作が可能かチェックする関数 `buildSettingsDiagnosticTest()` を用意しています。  
...
> * **※3**(3)：ビルド設定関係のファイル一式と下記ユーティリティの `cpuid` のソースだけあすれば、`buildSettingsDiagnosticTest()` を実行できます。  
> 本ライブラリが動作しない環境であっても、コンパイラの状態や実行環境の確認に使うことができます。  
> * **※4**：ライブラリのネームスペースを独自にカスタマイズできます。  
> * **※5**：ライブラリの挙動を設定するためのコンパイラスイッチとしてのマクロを定義しています。
...
> ＜注釈＞  
> * **※1**：アラインメント計算、アラインメント対応の標準メモリアロケータに対応します。  
...
> * **※2**：各種メモリアロケータのための基本的な定数（デフォルトアラインメントなど）、および、明示的なコンストラクタ／デストラクタ呼び出し関数に対応します。  
...
> * いずれも、「指定サイズのメモリ確保」、「型指定のメモリ確保とコンストラクタ呼び出し」、「配列型指定のメモリ確保とコンストラクタ呼び出し」（`new[]`演算子と異なり、デフォルトコンストラクタ以外の呼び出しも可）、「メモリ破棄とデストラクタ呼び出し」に対応しています。  
> * いずれも、テンプレート引数に `spinLock` や `std::mutex` などを与えることにより、排他制御を有効化し、スレッドセーフなメモリ確保が可能となります。デフォルトは `dummyLock` で、排他制御しません。  
> * いずれも、任意のアラインメント指定に対応します。  
...
* `gasha/poly_allocator.h` **多態アロケータ** ※2  

> ＜説明＞  
> * **【注意】**：この機能を有効化すると、**標準 `new`/`delete` 演算子をオーバーライドします**。使用時の際には、プロジェクト全体の方針を決めて、慎重に導入する必要があります。  
> * 標準new/delete演算子によるメモリ確保／解放に対して、任意のアロケータを適用することができます。  
...
* 【検討事項：統合メモリマネージャ】`gasha/memory_manager.h` **統合メモリマネージャ**  
> * ヒープメモリ対応。  
