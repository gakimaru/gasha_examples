...
任意に追加しても問題ありません。  
* `DEBUG`          ... デバッグ機能あり、最適化：なし、ファイルシステム：ローカル／ROM切り替え（データで設定）、ユニットテスト：可  
* `DEBUG_MODERATE` ... デバッグ機能あり、最適化：小、  ファイルシステム：ローカル／ROM切り替え（データで設定）、ユニットテスト：可  
* `DEBUG_OPT`      ... デバッグ機能あり、最適化：最大、ファイルシステム：ローカル／ROM切り替え（データで設定）、ユニットテスト：可  
* `UNIT_TEST`      ... デバッグ機能なし、最適化：最大、ファイルシステム：ローカル／ROM切り替え（データで設定）、ユニットテスト：専用  
* `RELEASE`        ... デバッグ機能なし、最適化：最大、ファイルシステム：ROMのみ、                              ユニットテスト：不可  

####ビルド構成に関係するマクロ####  
以下、ビルド構成に応じて指定されるマクロです。  
このマクロに基づいて、処理が有効化／無効化されてライブラリがビルドされます。  
ライブラリを使用するプロジェクトでも使用してよいマクロです。  
* `DEBUG_FEATURE`           ... デバッグ機能有効化  
* `NO_OPTIMIZED`            ... 最適化なし *1  
* `OPTIMIZED_MODERATELY`    ... 適度に最適化 *1  
* `OPTIMIZED`               ... 最大限の最適化 *1  
* `UNITE_TEST_ENABLED`      ... ユニットテスト（の仕組みが）有効  
* `FILE_SYSTEM_IS_ROM`      ... ROM専用ファイルシステム  
* `FILE_SYSTEM_IS_FLEXIBLE` ... ローカル／ROM切り替えファイルシステム（ローカルデータで設定）  
