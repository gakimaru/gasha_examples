...
> なお、ライブラリのソースコードにはMITライセンスを適用しています。  
...
> コードサイズを抑えることよりも、パフォーマンスを優先した設計としています。  
> テンプレートの柔軟性を活用し、C++の「ゼロオーバーヘッドの原則」に倣って、不要な処理がパフォーマンスに影響しないように設計しています。  
> 例えば、「優先度付きキュー」などの幾つかのクラスはマルチスレッドで排他制御を行いますが、シングルスレッドで利用する場合には、排他制御の処理コードがコンパイル時に完全に消滅します。条件分岐もありません。（注：インライン展開などのコンパイラの最適化が十分に機能することによって実現します）  
