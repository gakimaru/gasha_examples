...
> 主にゲーム開発での利用を想定し、品質と開発効率の向上を目的とした基本的要素を組み込んだライブラリです。  
...
###パフォーマンスと生産性を重視した設計です。###
> ライブラリの多くをテンプレートクラス／関数で構成しています。  
> 
> * コンテナクラスやシングルトンクラスなどの多くのクラスで、ロック制御やデバッグ機能をポリシー化しており、必要に応じて機能を有効化／無効化できるように構成しています。  
> * これにより、煩雑な条件分岐を行わずにマルチスレッド／シングルスレッド（もしくはデバッグ機能有効化時／無効化時）の両方に最適化することができ、高機能で汎用的なクラスライブラリを実現しています。  
> * また、メタプログラミングの対応を強化しており、コンパイル時のCRC計算や素数計算などにより、実行時のパフォーマンス向上を図ることができます。  
> * 反面、実行コードサイズや、コンパイルの負担（コンパイル時間）が増大します。Visual C++ のインテリセンスが重くなる場合もあります。  

###リミテッドな処理を基本としています。###
> STLコンテナのように暗黙的で無制限なメモリ確保をせず、あらかじめ最大サイズを決めて扱う処理を基本としています。  
> 
> * リミテッドな処理はゲーム制作に向いています。  
> リソース制限、パフォーマンスの安定化などに寄与します。  
...
###マルチスレッドに最適化しています。###
> C++11によって標準化されアトミック操作を活用し、プラットフォーム非依存のロックフリーアルゴリズムやスピンロック／共有スピンロックを実現しています。  
> 
> * マルチコアに対する最適化にも有効です。  
> * 前述のとおり、コンテナクラスなどの多数のクラスでロック制御をポリシー化しており、それら多数のクラスをマルチスレッドで共通利用できます。  

###マルチプラットフォームで共通利用可能なソースコードの強化を促します。###
...
> * 現状では、Visual C++ 12.0（Visual Studio 2013）の Win32, x64、および、GCC4.7 以降の x86, x64 に対応しています。  
...
###複数プロジェクトでライブラリを共通利用することを想定しています。###
...
##ユーティリティ系##
* `gasha/utility.h` ... ［テンプレート関数含む］**汎用ユーティリティ** ※1  
...
* `gasha/lf_stack.h` ... ［テンプレートクラス］**ロックフリースタック** ※1 ※T  
* `gasha/lf_queue.h` ... ［テンプレートクラス］**ロックフリーキュー** ※1 ※T  
* `gasha/tagged_ptr.h` ... ［テンプレートクラス］**タグ付きポインター**  
...
* `gasha/allocator_common.h` ... ［テンプレート関数含む］**アロケータ共通設定・処理** ※2 ※T  
...
##シリアライズ系##
* `gasha/serialization.h` ... ［テンプレートクラス］**シリアライズ** ※1  
...
###ターゲットプラットフォーム###
現状、x86系Windows、x64系Windows、Cygwin、x64系Linux に対応しています。  
    ...
    [gasha_examples(gasha_proj)] ... ライブラリサンプルプログラム／ビルドプロジェクト用リポジトリ
     ...
     |-[sub]                     ... サブモジュール用
        ...
        `-[gasha]                ... サブモジュール：ライブラリ本体用リポジトリ
           ...
           |     |- gasha_x64.a                 ... x64リリースビルド用
           |     `- gasha_x64_debug.a           ... x64デバッグビルド用
           |                                        （x86版はCygwinで開発し、x64版はLinuxで開発しています）
           |
           `-[proj]
              |
...
> > `project/sub/gasha/lib/gcc/gasha_x64.a` ... x64リリースビルド用  
> > `project/sub/gasha/lib/gcc/gasha_x64_debug.a` ... x64デバッグビルド用  
