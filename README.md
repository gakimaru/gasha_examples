...
> ライブラリの動作を確認するための、多数音サンプルプログラムを扱っています。  
...
> メモリ安全なコンテナとアルゴリズム（暗黙的なメモリ確保・解放を行わい）、多様なメモリアロケータ、利便性の高いデバッグ機能、全般的なスレッドセーフ設計などにより、生産性の向上と品質向上、パフォーマンスの向上に寄与します。  
...
> 
> * 例えば、各種コンテナクラスはマルチスレッド制御のためのインターフェースを備えていますが、マルチスレッド制御が不要ななら、コンパイル時にそのコードが消滅するように構成しています。  
...
> 今後のライブラリの追加実装は、関心と余力に応じて、気まぐれに行っていきます。  
...
> * **※4(2)**：`alignas`, `thread_local`, `__PRETTY_FUNCTION__` といった偽装処理を個別に無効化できます。他のライブラリと競合するような場合にカスタマイズできます。
> なお、ライブラリファイルのビルド時は、このカスタマイズと無関係に偽装処理が有効化されます。  
> * **※5**：ライブラリの挙動を設定できます。コンパイラスイッチとしてのマクロを有効化／無効化することで設定します。  
> ユーザー側で任意にライブラリの挙動をカスタマイズできます。  
> 例えば、標準 new/delete 演算子をオーバーロードする「多態アロケータ」機能の有効化／無効化を指定するといったことが可能です。  
