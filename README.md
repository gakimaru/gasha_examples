...
* `gasha/serialization/friends.h` ... **シリアライズ対象クラス／構造体のフレンド宣言用** ※2  
* `gasha/archive/output_binary_archive.h` ... **バイナリ形式シリアライズデータ書き込みクラス** ※3  
* `gasha/archive/input_binary_archive.h` ... **バイナリ形式シリアライズデータ読み込みクラス** ※3  
* `gasha/archive/output_text_archive.h` ... **テキスト形式シリアライズデータ書き込みクラス** ※3  
...
> * **※3**：シリアライズ処理を実際にどのような形式で扱うかは、別途アーカイブクラスを指定することで決まります。  
