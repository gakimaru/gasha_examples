...
**【整列状態確認】**  
* `gasha/is_ordered.h` ［テンプレート］**整列状態確認** *3  
...
####マルチスレッド：共有データ####
* `gasha/shared_pool_allocator.h` ［テンプレート］**共有プールアロケータ** *1 *2  
* `gasha/shared_stack.h` ［テンプレート］**共有スタック** *1 *2  
* `gasha/shared_queue.h` ［テンプレート］**共有キュー** *1 *2  
...
> > * *2 コンパイル時間およびクラス修正時の広範囲な再コンパイルの影響を考慮し、ソースファイルは宣言部（.h）、インライン関数／テンプレート関数実装部（.inl）、関数実装部（.cpp.h）に分けており、それぞれ適切な場所でインクルードする必要があります。（面倒なら全部まとめてインクルードしても使えます）  
> 
> 資料  
> * **[プログラム最適化Tips.pdf][プログラム最適化Tips.pdf]**  
> ［データ構造とアルゴリズムによる最適化］-［マルチスレッドを最適化するアルゴリズム］  
...
####マルチスレッド：ロックフリー共有データ####
* `gasha/lf_pool_allocator.h` ［テンプレート］**ロックフリープールアロケータ** *2  
* `gasha/lf_stack.h` ［テンプレート］**ロックフリースタック** *1 *2  
* `gasha/lf_queue.h` ［テンプレート］**ロックフリーキュー** *1 *2  
...
####擬似コンテナ####
**【配列】**  
* `gasha/dynamic_array.h` ［テンプレート］**動的配列** *1 *3  
* `gasha/ring_buffer.h` ［テンプレート］**リングバッファ** *1 *3  

**【連結リスト】**  
* `gasha/linked_list.h` ［テンプレート］**双方向連結リスト** *2 *3  
* `gasha/singly_linked_list.h` ［テンプレート］**片方向連結リスト** *2 *3  

**【二分探索木】**  
* `gasha/rb_tree.h` ［テンプレート］**赤黒木** *2 *3  

> 説明  
> * メモリ管理を行わず、外部のデータを操作する疑似コンテナです。  
> * STLライクなインターフェースで操作可能ですが、データ構造とアルゴリズムを分離して扱えるため、自由度の高い応用が可能です。  
> * 使用時の設定により、マルチスレッドセーフ（ロック制御）の仕組みを有効化できます。  
> 
> > 注釈  
> > * *1 配列系コンテナは、外部から既存の配列を渡して扱います。  
> > * *2 連結リスト／木系コンテナは、外部で生成したノードの連結・解除を扱います。  
> > * *3 コンパイル時間およびクラス修正時の広範囲な再コンパイルの影響を考慮し、ソースファイルは宣言部（.h）、インライン関数／テンプレート関数実装部（.inl）、関数実装部（.cpp.h）に分けており、それぞれ適切な場所でインクルードする必要があります。（面倒なら全部まとめてインクルードしても使えます）  
> 
> 資料  
> * **[プログラム最適化Tips.pdf][プログラム最適化Tips.pdf]**  
...
####コンテナ####
* `gasha/hash_table.h` ［テンプレート］**開番地法ハッシュテーブル** *1  
* `gasha/binary_heap.h` ［テンプレート］**二分ヒープ** *1  

####コンテナアダプタ####
* `gasha/priority_queue.h` ［テンプレート］**優先度付きキュー** *1  
