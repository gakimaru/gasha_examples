...
* `GASHA_HAS_DEBUG_LOG`           ... デバッグログ有効化  
* `GASHA_ASSERTION_IS_ENABLED`    ... アサーション有効化  
* `GASHA_DEV_TOOLS_IS_AVAILABLE`  ... 開発ツール利用可能 ※1  
* `GASHA_NO_OPTIMIZED`            ... 最適化なし ※2  
* `GASHA_OPTIMIZED_MODERATELY`    ... 適度に最適化 ※2  
* `GASHA_OPTIMIZED`               ... 最大限の最適化 ※2  
...
* `GASHA_IS_REGRESSION_TEST`      ... 回帰テストモード有効 ※3  
* `GASHA_HAS_SYMBOLS`             ... シンボル情報あり ※4  
* `GASHA_IS_STRIPPED_SYMBOLS`     ... シンボル情報なし ※4  

> 注釈
> * **※1**：現状、このオプションはVisual Studioの出力ウインドウを使えるかどうかの判定に用いています。  
> * **※2**：これらは、ビルドの最適化状態を示すことを目的としたマクロであり、実際にこのマクロに基づいて最適化するわけではありません。  
> ビルド構成ごとに、別途最適化オプションが設定されます。  
> なお、このマクロに基づいて、最適化コードの切り替えを行う処理を組んでも構いません。  
> * **※3**：「回帰テスト」はユニットテストの自動実行です。  
> * **※4**：これもビルドの状態を示すことを目的としたマクロであり、実際にこのマクロに基づいてシンボル情報の組み込み／削除を行うわけではありません。  
