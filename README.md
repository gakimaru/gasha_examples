...
> 
> $ pwd
> ***/gasha_examples         ... gasha_examples ディレクトリに移動して下さい。  
> $ nkf -Lu --overwrite *.sh ... 最低限のシェルスクリプト（.sh ファイル）は手動で変換して下さい。  
> $ chmod +x *.sh            ... 同様に、最低限のシェルスクリプトには実効権限も付与して下さい。  
> $ conv_sh_lf.sh            ... 一括変換のシェルスクリプトを実行して下さい。  
> $ conv_makefile_lf.sh      ... Makefile の改行コードも一括変換して下さい。  
>
