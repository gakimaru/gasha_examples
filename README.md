...
> ＜注釈＞  
> * **※1**：べき乗、素数判定、素数計算、ビット数／MSB／LSB計算といった基本的な算術系処理を扱います。  
> * **※2**(1)：SSE命令や逆数を使った高速な四則演算・平方根を用意しています。  
> 通常演算の際に、値を`fastArith`クラスでラップするだけで、高速演算が適用されるインターフェースです。  
> * **※2**(2)：SSE命令などのプラットフォーム固有の演算命令を隠蔽しているので、ソースコードレベルで互換性のある高速演算として利用できます。  
> * **※2**(3)：SSE命令有効化状態で `float` 型に高速演算を適用すると、内部で `__m128` 型で値を保持します。
> 演算は高速になりますが、情報量が増えることには注意が必要です。  
> * **※2**(4)：`fastArith`クラスを用いると、`__m128` `__m128d` `__m256` `__m256d` のSIMD四則演算を四則演算子で演算できます。  
> * **※2**(5)：`fastArith`クラスを本ライブラリの平方根関数 `sqrt` に適用すると、高速演算を行います。  
> * **※2**(6)：高速演算は必ず効果のあるものではなく、コンパイラの最適化に任せた方が良い結果になることも多いので、
> コンパイル・実行環境に合わせて、よく検証した上で使用する必要があります。  
> * **※2**(7)：逆数を使った最適化は、局所的なスループットの改善にはなっても、結果としてあまり効果が出ません。
> やはりコンパイラの最適化の効果が絶大です。  
...
* 【検討事項】`gasha/aggregate.h` ［テンプレートクラス］**集合**  
