...
> C++11を基盤に開発しているため、プラットフォーム／コンパイラ依存のコードが少ない点、および、それによって標準化されたマルチスレッド処理対応が多い点も特徴です。  

**基本的なアルゴリズム、コンテナ、メモリ管理、デバッグ機能などを用意しています。**  
> STLライクのコンテナは、データ構造とアルゴリズムを分離した構成としており、ユーザー独自のデータを一時的にコンテナ化して操作するなど、柔軟に活用することができます。  
...
> 説明  
> * べき乗、素数判定、素数計算、ビット数計測といった基本的な算術系処理を扱います。  
